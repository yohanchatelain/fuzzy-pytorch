...
void time_sr(const char methodname[], const size_t ntests, const size_t nreps,
  ...
  data->max = -INFINITY;
  data->mean = 0;
  double *timings = (double*)malloc(ntests * sizeof(double));
  for (i=0; i<ntests; i++) {
...
void time_sr_mpfr(const char methodname[], const size_t ntests, const size_t nreps,
                  struct timingdata *data,
...
void time_mpfr(const char  methodname[], const size_t ntests, const size_t nreps,
               struct timingdata *data,
               int (* mpfr_fun)(mpfr_ptr,
                                mpfr_srcptr,
                                mpfr_srcptr,
...
void time_sr_single_arg(const char methodname[], const size_t ntests,
                        const size_t nreps,
                        struct timingdata *data,
...
/* Time methods for stochastic rounding based on MPFR. */
void time_sr_mpfr_single_arg(const char methodname[],
...
/* Time MPFR functions for reference. */
void time_mpfr_single_arg(const char methodname[],
                          const size_t ntests, const size_t nreps,
                          struct timingdata *data,
                          int (* mpfr_fun)(mpfr_ptr,
                                           mpfr_srcptr,
...
#define get_terminator(i, nalgs) (i==5||i==10||i==15? "&& " : i<nalgs-1? "& " : "")
...
int main () {
  ...
  char* algname = (char*)malloc(50 * sizeof(char));
  for (i=0; i<3; i++) {
  ...
  const char *terminator = (char*)malloc(10 * sizeof(*terminator));
  FILE * f = stdout;

  size_t reference;
  fprintf(f, "min       & ");
  for (i=0; i<nalgs; i++) {
    terminator = get_terminator(i, nalgs);
