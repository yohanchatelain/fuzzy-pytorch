...
#define DBL_TRUE_MIN std::numeric_limits<double>::denorm_min()
...
void perform_a_test(double a, double b, double expected1, double expected2,
  ...
  double x;
  size_t *counters = (size_t*)malloc(2*sizeof(size_t));
  ...
  for (size_t j=0; j<nreps; j++) {
    ...
    if (x == expected1)
      ...
      printf("\n\n[ERROR] Not rounding to either nearest fp values %.13a.\n", x);
      exit(10);
    }
  }
  ...
  for (i=0; i<ntests; i++) {
    ...
    for (j=0; j<nreps; j++) {
      ...
      if (x == results.xd)
        ...
        printf("\n\n[ERROR] Not rounding to either nearest fp values %.13a.", x);
        exit(10);
      }
    }
  ...
  for (i=0; i<ntests; i++) {
    ...
    for (j=0; j<nreps; j++) {
      ...
      if (x == results.xd)
        ...
        printf("[ERROR] Not rounding to either nearest fp values %.13a.", x);
        exit(10);
      }
    }
